...
#endif
extern "C" EXPORTS_DEMO void HH_Entrance(float gNa, float gK, float gKM, float gKv, float gCa, double Mtime, double tempVB, double TimeStep, vector<float> m_I, int FlagParameter[], float gL, float C, float *pArrayA);
#endif
/////////////////////////////////////////////////////
void Calculation_I(CCell Cell_B, double Mtime, int FlagParameter[], float gL, float C, float *pArrayA, vector<float> m_I)
{
	...
	for (int k = 0; k < Mtime / Cell_B.GetTimeStep(); k++)
	{
		...
		y_preB = tempVB + Cell_B.GetTimeStep()*(gL*(Cell_B.GetStart() - tempVB) + m_I[k]
		...
		y_nextB = tempVB + Cell_B.GetTimeStep()*(gL*(Cell_B.GetStart() - y_preB) + m_I[k]
			- cell_B_Na_I - cell_B_K_I - cell_B_Ca_I - cell_B_KM_I - cell_B_Kv_I)/C;
...
void HH_Entrance(float gNa, float gK, float gKM, float gKv, float gCa, double Mtime, double tempVB, double TimeStep, vector<float> m_I, int FlagParameter[], float gL, float C, float *pArrayA)
{
	...
	Calculation_I(Cell_D, Mtime, FlagParameter, gL, C, pArrayA, m_I);
